...
def visualize_2d_multi(clusters, colors=None, labels=None, style='seaborn-v0_8',connection=False,method="umap"):
    assert isinstance(clusters, list)
    for cluster in clusters:
        assert isinstance(cluster, np.ndarray)
        assert cluster.shape[1] == 2
    ...
    if connection:
        ...
        for cluster, color, label in zip(clusters, colors, labels):
            idx+=1
            if idx <=3:
                plt.scatter(cluster[:, 0], cluster[:, 1], s=5, marker='o', c=color, label=label, alpha=1)
            else:
                plt.scatter(cluster[:, 0], cluster[:, 1], marker='x', c=color, label=label, alpha=0.5)

        for i in range(len(clusters[0])):
            plt.plot(
                [clusters[0][i, 0], clusters[3][i, 0]],
                [clusters[0][i, 1], clusters[3][i, 1]],
                c="k",
                linewidth=0.2,
                alpha=0.05,
            )
        for i in range(len(clusters[1])):
            plt.plot(
                [clusters[1][i, 0], clusters[4][i, 0]],
                [clusters[1][i, 1], clusters[4][i, 1]],
                c="k",
                linewidth=0.2,
                alpha=0.05,
            )
        for i in range(len(clusters[2])):
            plt.plot(
                [clusters[2][i, 0], clusters[5][i, 0]],
                [clusters[2][i, 1], clusters[5][i, 1]],
                c="k",
                linewidth=0.2,
                alpha=0.05,
            )
    ...
    modality_gap1 = clusters[0].mean(axis=0) - clusters[3].mean(axis=0)
    modality_gap2 = clusters[1].mean(axis=0) - clusters[4].mean(axis=0)
    modality_gap3 = clusters[2].mean(axis=0) - clusters[5].mean(axis=0)
    delta1 = np.linalg.norm(modality_gap1)
    delta2 = np.linalg.norm(modality_gap2)
    delta3 = np.linalg.norm(modality_gap3)
    mean_delta = (delta1+delta2+delta3)/3
    print('method: {}, gap1: {}, delta1: {}, gap2: {}, delta2: {}, gap3: {}, delta3: {}, mean delta: {}'.format(method, str(modality_gap1),str(delta1),str(modality_gap2),str(delta2),str(modality_gap3),str(delta3),str(mean_delta)))
...
def reduce_and_visualize_multi(
    ...
    features_reduce = dim_reduce(features, n_dim=n_dim, methods=methods)
    idx0 = fea_list[0].shape[0]
    idx1 = fea_list[0].shape[0]+fea_list[1].shape[0]
    idx2 = fea_list[0].shape[0]+fea_list[1].shape[0]+fea_list[2].shape[0]
    idx3 = fea_list[0].shape[0]+fea_list[1].shape[0]+fea_list[2].shape[0]+fea_list[3].shape[0]
    idx4 = fea_list[0].shape[0]+fea_list[1].shape[0]+fea_list[2].shape[0]+fea_list[3].shape[0]+fea_list[4].shape[0]
    idx5 = fea_list[0].shape[0]+fea_list[1].shape[0]+fea_list[2].shape[0]+fea_list[3].shape[0]+fea_list[4].shape[0]+fea_list[5].shape[0]

    for i, method in enumerate(methods):
        image_label0_features_reduce = features_reduce[method][:idx0]
        image_label1_features_reduce = features_reduce[method][idx0:idx1]
        image_label2_features_reduce = features_reduce[method][idx1:idx2]

        text_label0_features_reduce = features_reduce[method][idx2:idx3]
        text_label1_features_reduce = features_reduce[method][idx3:idx4]
        text_label2_features_reduce = features_reduce[method][idx4:idx5]
        
        
        eval(f"visualize_{n_dim}d_multi")(
            [image_label0_features_reduce, image_label1_features_reduce,image_label2_features_reduce,text_label0_features_reduce,text_label1_features_reduce,text_label2_features_reduce],
            colors=colors,
            style=style,
            connection=connection,
            method=method,

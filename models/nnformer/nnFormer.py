...
from models.psat import PlaneSliceAttentionTransformer
...
class final_proj(nn.Module):
    def __init__(self,dim,patch_size):
        super().__init__()
        self.down=nn.Conv3d(dim,512,patch_size,0)
...
class final_restore(nn.Module):
    def __init__(self,dim,patch_size):
        super().__init__()
        self.up=nn.ConvTranspose3d(dim,1,patch_size,patch_size)
...
class nnFormer(SegmentationNetwork):
    ...
    def __init__(self, crop_size=[64,128,128],
        ...
        # for pretrain
        self.res = final_restore(embed_dim,patch_size=patch_size)
        self.psat = PlaneSliceAttentionTransformer(hidden_size=embedding_dim*8)
        self.queries = nn.Embedding(300, embedding_dim*8)
        nn.init.normal_(self.queries.weight.data)
        self.proj = nn.Linear(768,512)
        #Gram matrix 
        A = torch.rand(512, 512).to(torch.device("cuda"))
        init_M = 0.5*(A+A.t())
        init_M = init_M + torch.eye(512).to(torch.device("cuda"))
        self.L = torch.nn.Parameter(init_M) # 1024 1024
    ...
    def forward_pretrain(self, x, plane, idx_slice):
        bs = x.shape[0]
        skips = self.model_down(x)
        neck=skips[-1] # bs 768 4 4 4
        out=self.decoder(neck,skips)
        res_out = self.res(out[-1])

        
        query = self.queries.weight[:, None, :].repeat(1, bs, 1).transpose(0,1)
        med_proj = self.psat(query,neck.flatten(2).transpose(1,2),plane,idx_slice)
        med_proj = self.proj(med_proj)[:,0,:]
        return med_proj, x, res_out # bs 300 768, bs 1 128 128 128